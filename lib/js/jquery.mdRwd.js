    ...
    $.fn.mdRwd = function(options) {
		...
		if(opts.stateful){
                ...
                $.each(opts.layouts, function(currLayout, layoutDetails){
                    ...
                    wsMessage.initLayouts[currLayout].count = 0;
                });
                _wsSend(wsMessage);
            }     
		...
		if(wsMessage.statusCode!=mdRwdStatusCodeOk){
            ...
            function _connVc(changeLayout, oldLayout){
                $("#mdRwdConnectVc").hide();
                $("#mdRwdConnectVcBox .mdRwdLoadingBox").show();
                wsMessage = {"cmd": "connVc", "vcId": vcId, "changeLayout": changeLayout, "client": {"device": device, "layout":  {"id": layout.id}}};
                if(changeLayout){
                    wsMessage.client.oldLayout = {"id": oldLayout.id};
                }
                if(opts.stateful){
                    wsMessage.client.layout.components = layout.componentsArray;
                }
                ...
                if(opts.layouts[layoutId]){
                    if(layoutId != layout.id){
                        if($.inArray(device.id, opts.layouts[layoutId].devicesArray)>-1){
                            ...
                            eventsAffectedComponents = {};
                            opts.mdRwdOnChangeLayout(oldLayout, newLayout);
                            layout.mdRwdOnChangeLayout(oldLayout, newLayout);
                            layout = newLayout;
                            firstConnVcOccurred = false;
                            firstConnVcFailed = false;
                            _connVc(true, oldLayout);
                        }else{
