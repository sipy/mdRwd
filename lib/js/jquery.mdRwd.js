...
(function($) {
    ...
    $.fn.mdRwd = function(options) {
	    ...
	    var events = {
		"mdRwd":    ["newVc", "connVc", "triggerVcEvent", "rmVcClient", "rmVc"],
		...
		"mdRwdOnChangeLayout": function(oldLayout, newLayout){},
                ...
                "html": {
                    ...
                    "window": {
                        ...
                        "content": {
                            ...
                            "vcLabel": "Your ID",
                            "loadingImgPath": "img/loading.gif",
	    ...
	    function _mdRwdDestroy(){
            ...
            function _mdRwdWaitWsDisconnAndDestroy(){
               setTimeout(function(){
                    if(ws.readyState === 3){
                        _mdRwdDestroyHelper();
                    }else{
		...
		var outgoing = {};
		var incoming = {};
                $.each(events, function(ev, eventArray){
                    outgoing[ev] = "";
                    incoming[ev] = {};
                    if(eventArray.length>0){
                        $.each(eventArray, function(index, event){
                           incoming[ev][event] = function(){};
                        });
                    }
                });
                var defaultsLayout = {
                ...
                if(!$.isEmptyObject(opts.layouts)){
                    $.each(opts.layouts, function(currLayout, layoutDetails){
                        ...
                        $.each(layoutDetails.events.outgoing, function(typeEvent, events){
                            layoutDetails.events.outgoing[typeEvent+"Obj"] = {};
                            var tmpObj = {};
                            $.each(events.match(/(?:[^,(]|\([^)]*\))+/g), function(index, strToParse){
                                tmpObj = _mdRwdParseOutEvent(currLayout, typeEvent, strToParse);
                                if(opts.stateful){
                                    layoutDetails.events.outgoing[typeEvent+"Obj"][tmpObj.event] = {};
                                    layoutDetails.events.outgoing[typeEvent+"Obj"][tmpObj.event].affectedComponents = tmpObj.affectedComponents;
                                }
                            });
                        });
                        //override defaults with user parameters
		...
		var deviceLayoutsIds = [];
                $.each(opts.layouts, function(currLayout, currLayoutDetails){
                    if($.inArray(device.id, currLayoutDetails.devicesArray)>-1){
                ...
                if(deviceLayoutsIds.length > 1){
                    ...
                    _mdRwdError('The "' + device.id + '" device has no associated layouts');
                    return false;
                }		
	    }
		...
		wsMessage = { "cmd": "triggerVcEvent", "vcId": vcId, "event": ev,  "client": {"layout":  {"id": layout.id}}};
		if(opts.stateful){
				...
				startEvent = {
				    "type": ev.type,
                                    "eventData":{
                                        ...
                                        "target": ev.gesture.startEvent.target.id,
				...
				touchEvent = {
				    "type": ev.type,
                                    "eventData":{
                                        ...
                                        "screenDim": {"height": $(window).height(), "width": $(window).width()},
		...
		if (customEvents) {
			...
			if(!ev.namespace || ev.namespace != mdRwdNameSpace){
			    var customEvent = {
                                "type":ev.type,
                                "eventData":ev.eventData?ev.eventData:{}
                            };
                            if(opts.stateful){
            ...
            function _mdRwdWaitWsConn(){
                setTimeout(function(){
                    if(ws.readyState === 1){
                        ...
                        $("#mdRwdLayoutChoice").html('<div class="mdRwdWarningMessage">' + opts.html.window.content.warningMessages[1006] + '</div>');
		...
		var wsMessage = JSON.parse(ev.data);
                var event = {
                    ...
                    "type": wsMessage.cmd=="triggerVcEvent"?wsMessage.event.type:wsMessage.cmd,
                    "eventData": wsMessage.cmd=="triggerVcEvent"?wsMessage.event.eventData:null,
                    "client": wsMessage.client,
                    "vcId": wsMessage.vcId
                };
                switch (wsMessage.cmd) {
                    case "newVc":
                        _newVcHandler(event, wsMessage);
                        break;
                    case "connVc":
                        _connVcHandler(event, wsMessage);
                        break;
                    case "triggerVcEvent":
                        _triggerVcEventHandler(event, wsMessage);
                        break;
                    case "rmVcClient":
                        _rmVcClientHandler(event, wsMessage);
                        break;
                    case "rmVc":
                        _rmVcHandler(event, wsMessage);
                        break;
                    default:
                        break;
                }
		opts.mdRwdOnWsMessage(ev, wsMessage);
		layout.mdRwdOnWsMessage(ev, wsMessage);
	    }
		...
		wsMessage = {"cmd": "newVc", "vcId": "", "client": {"layout": {"id": layout.id}}};
		if(opts.stateful){
	    ...
	    function _newVcHandler(event, wsMessage){
                $("#mdRwdVcBox .mdRwdLoadingBox").hide();
                $("#mdRwdVcId").show();
                vcId = wsMessage.vcId;
                if(clientId==0 && wsMessage.client.isMe){
		...
		if(wsMessage.statusCode!=mdRwdStatusCodeOk){
            ...
            function _connVc(changeLayout){
                $("#mdRwdConnectVc").hide();
                $("#mdRwdConnectVcBox .mdRwdLoadingBox").show();
                wsMessage = {"cmd": "connVc", "vcId": vcId, "changeLayout": changeLayout, "client": {"layout":  {"id": layout.id}}};
                if(opts.stateful){
                    wsMessage.client.layout.components = layout.componentsArray;
                }
                _wsSend(wsMessage);
	    ...
	    function _connVcHandler(event, wsMessage){
                if(clientId==0 && wsMessage.client.isMe){
                    clientId = wsMessage.client.id;
                }
		if(wsMessage.statusCode!=mdRwdStatusCodeOk){
                    if(wsMessage.lock){
                        ...
                        if(timeConnVcFail - timeFirtsConnVcFail > opts.connVcTimeout){
                            ...
                            setTimeout(function(){_connVc(false)}, opts.connVcInterval);
                        }
                    }else{
                        opts.mdRwdOnWarning(wsMessage.statusCode);
                        layout.mdRwdOnWarning(wsMessage.statusCode);
		...
		$(document).trigger(event);
            ...
            function _setLocalLayoutState(components){
                if(components){
                    $.each(components, function(currComp, compDetails){
                        _setLocalVcCompState(currComp, compDetails.state, false);
                    });
                }
            }

            function _setLocalVcCompState(compId, state, sendToServer){
                if(opts.components[compId]){
                    opts.components[compId].state = state;  
                    sendToServer = typeof sendToServer !== 'undefined' ? sendToServer : false;
                    if(sendToServer){
            ...
            function _setServerVcCompState(compId){
                ...
                wsMessage = {"cmd": "setVcCompState", "vcId": vcId, "componentState":  componentState, "client": {"layout": {"id":layout.id, "components": layout.componentsArray}}};
                _wsSend(wsMessage);
            }
                ...
                if(opts.layouts[layoutId]){
                    if(layoutId != layout.id){
                        if($.inArray(device.id, opts.layouts[layoutId].devicesArray)>-1){
                            ...
                            _mdRwdDestroyEventsCallbacks();
                            eventsAffectedComponents = {};
                            opts.mdRwdOnChangeLayout(layout.id, layoutId);
                            layout.mdRwdOnChangeLayout(layout.id, layoutId);
                            layout = opts.layouts[layoutId];
                            firstConnVcOccurred = false;
                            firstConnVcFailed = false;
                            _connVc(true);
                        }else{
	    ...
	    if(opts.stateful){
                return {
                    ...
                    "setVcCompState": _setLocalVcCompState,
                    "getVcCompState": _getLocalVcCompState,
